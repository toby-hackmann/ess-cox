...
knitr::opts_chunk$set(
  ...
  results = 'markup',   # Optional: Hide text output (don't use if you want to see tables)
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, output=TRUE, dev='png', dpi=300, fig-cap="Distribution of 4 prognostic factors in the colon dataset. Adhere: adhesion of the tumor. Treatment with three levels: Obs - Observation; Lev: levimasole; Lev+5FU - levimasole+5-fluorouracil. Age in years. Nodes: number of lymph nodes with detectable cancer."}


source(here::here("code", "compile.R"))
...
```{r, echo=FALSE, fig.align="center", fig.cap="The 95% Confidence Interval of a Cox model prediction for a patient with `Lev+5FU` (Levimasole + Fluorouracil) treatment is smaller than the 95% Confidence Interval of the Kaplan-Meier curve for the same treatment. The effect is most visible early on in follow-up, e.g. at 500 days.", output = TRUE}

knitr::include_graphics(here::here("images", "compare_ci.png"))
```
...
# Supplement 7: In logistic regression, effective sample size is symmetric
```{r, output = TRUE, echo = TRUE}

X <- rnorm(10000, 0, 1)
odds_ratio <- 2

odds <- exp(log(odds_ratio)*X)
p <- odds/(odds+1)
outcome <- rbinom(10000, 1, p)

glm <- glm(outcome ~ X, family = binomial(link = "logit"))
...
cov.beta <- vcov(glm)
X_LR0 <- model.matrix(glm)

mean <- data.frame(X=0)
SDplus <- data.frame(X=1)
SDmin <- data.frame(X=-1)

pred <- predict(glm, newdata = mean, type = "response", se.fit = TRUE)
neff_mean$pi.pred <- pred$fit
neff_mean$pi.se <- pred$se.fit
neff_mean$vary_xnew <- neff_mean$pi.pred*(1-neff_mean$pi.pred)
mm <- matrix(c(1, 0), nrow = 1, ncol = ncol(X_LR0))
neff_mean$n_eff_LR <- 1/((mm %*% (cov.beta %*% t(mm)))*neff_mean$vary_xnew)

pred <- predict(glm, newdata = SDplus, type = "response", se.fit = TRUE)
neff_plus$pi.pred <- pred$fit
neff_plus$pi.se <- pred$se.fit
neff_plus$vary_xnew <- neff_plus$pi.pred*(1-neff_plus$pi.pred)
mm <- matrix(c(1, 1), nrow = 1, ncol = ncol(X_LR0))
neff_plus$n_eff_LR <- 1/((mm %*% (cov.beta %*% t(mm)))*neff_plus$vary_xnew)

pred <- predict(glm, newdata = SDmin, type = "response", se.fit = TRUE)
neff_min$pi.pred <- pred$fit
neff_min$pi.se <- pred$se.fit
neff_min$vary_xnew <- neff_min$pi.pred*(1-neff_min$pi.pred)
mm <- matrix(c(1, -1), nrow = 1, ncol = ncol(X_LR0))
neff_min$n_eff_LR <- 1/((mm %*% (cov.beta %*% t(mm)))*neff_min$vary_xnew)

print(paste0("Effective sample size of a patient with mean X: ", round(neff_mean$n_eff_LR), ". Effective sample size of a patient with X one SD above the mean: ", round(neff_plus$n_eff_LR), ". Effective sample size of a patient with X one SD below the mean: ", round(neff_min$n_eff_LR), ". This shows that effective sample size can be higher than the total sample size in logistic regression, due to borrowing of information across patients."))
```

# Supplement 8: Heuristic explanation of higher than total sample size
...
# Supplement 9: Coefficient variance investigated
